...
class Signup extends React.Component {
  ...
  setUsernameClass() {
    ...
    return isValidUsername === true ? s.success : isValidUsername === false ? s.error : s.username;
  }

  setButtonStatus() {
    const { isValidUsername, isValidEmail, isValidPassword } = this.state;
    const status = isValidUsername && isValidEmail && isValidPassword;
  ...
  handleUsernameValidation() {
    ...
    if (username.length >= 3) {
      services.isValidUsername(username, isValid => this.setState({ isValidUsername: isValid }));
    }
